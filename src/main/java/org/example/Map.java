...
public class Map extends JPanel {
    ...
    // ширина, высота, количество ячеек по высоте и длине
    private int width, height, cellWidth, cellHeight;
    // ширина, высота ячейки, кол-во подряд значений для победы
    private int mode, fieldSizeX, fieldSizeY, winLen;
    private int[][] field; // массив кто куда сходил
    private boolean gameWork; // флаг статуса игры

    Map() {
        setBackground(Color.WHITE); // цвет фона
        addMouseListener(new MouseAdapter() {
            @Override
            public void mouseReleased(MouseEvent e) { // e - координаты щелчка
                //** слушает нажатие мышки **//
                if (gameWork) { // если игра идет
                    update(e); // делаем update
                }
            }
        });
    }
    ...
    private void initMap() {
        /** Метод создания массива, для сохранения координат**/
        field = new int[fieldSizeY][fieldSizeX];
    }

    void startNewGame(int mode, int sizeX, int sizeY, int winLen) {
        /** Метод начала игры с параметрами**/
        this.mode = mode;
    ...
    private void update(MouseEvent mouseEvent) {
        //передать ход сопернику и проверить победили или нет
    ...
    private boolean isValidCell(int x, int y) {
        // попал ли человек в игровое поле
